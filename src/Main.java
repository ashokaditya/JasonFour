...
public class Main {
	...
	public static void main(String[] args) throws Exception {
		...
		while (!Level.AreGoalsSatisfied()) {
			...
			ExecutePlans(solutions, serverMessages);
		}
	...
	private static void ExecutePlans(HashMap<Character, List<Node>> solutions, BufferedReader serverMessages) {
		...
		while (!needReplan) {
			...
			//TODO maybe make this a map instead with each agentName such that 
			//TODO the funcion if failed is prettier
			List<Command> updateActions = new LinkedList<Command>();
			...
			for (char agentName : Level.getAgentNames()) {
				...
				//This way the agent never reaches the NoOp state
				if(list == null || list.isEmpty()){
					needReplan = true;
					Level.setAgentFree(agentName);
					Level.setBoxFree(agentName);
			...
			int i = 0;
			if(!response.equals("")) response = response.substring(1, response.length());
			for (Command command : updateActions) {
				if(response.startsWith("true")){
					if(command != null){
						Level.update(Character.forDigit(i, 10), command);
						if(response.length() > 5 ) response = response.substring(6, response.length());
					}
				} else if(response.startsWith("false")) {
					needReplan = true;
					Level.setAgentFree(Character.forDigit(i, 10));
					Level.setBoxFree(Character.forDigit(i, 10));
					if(response.length() > 6 ) response = response.substring(7, response.length());
				}
				i++;
			}
		}
